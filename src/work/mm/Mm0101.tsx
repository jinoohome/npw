...
const Mm0101 = ({ item, activeComp, leftMode, userInfo }: Props) => {
   ...
   const breadcrumbItem = [{ name: "기준정보" }, { name: "거래처" }, { name: "거래처등록 (장례지원단)" }];
   ...
   const setGridData = async () => {
      await fetchWithLoading(async () => {
         try {
            // 병렬로 API 호출
            const [zz0005Data, result] = await Promise.all([
               ZZ_CODE({ coCd: "999", majorCode: "ZZ0005", div: "999" }),
               MM0101_S01()
            ]);
   ...
   const addGridRow = () => {
      ...
      grid.appendRow({ coCd: "100", paBpCd: "", bpType: "", bpDiv: "", bankCd: "", useYn: "Y" }, { at: 0 });
