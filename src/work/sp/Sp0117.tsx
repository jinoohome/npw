...
const Sp0104 = ({ item, activeComp, leftMode, userInfo, handleAddMenuClick, setSoNo}: Props) => {
   ...
   const SP0104_S01 = async (soNo: string) => {
      const param = {
         ...
         itemNm: inputValues.searchItemNm || "999",
      };

      const data = JSON.stringify(param);
      const result = await fetchPost("SP0117_S01", { data });
     ...
     const searchDiv = () => (
      <div className="bg-gray-100 rounded-lg p-5 search text-sm search">
         <div className="grid grid-cols-3  gap-y-3  justify-start w-[60%]">
            ...
            <InputComp title="품목명" ref={searchItemNmRef} value={inputValues.searchItemNm}   handleCallSearch={search} onChange={(e) => onInputChange("searchItemNm", e)} />
