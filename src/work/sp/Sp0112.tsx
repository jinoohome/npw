...
const Sp0108 = ({ item, activeComp, userInfo }: Props) => {
   const breadcrumbItem = [{ name: "수발주관리" }, { name: "수발주관리" }, { name: "일정확인" }];
   ...
   const fetchEventsForMonth = async (start: Date, end: Date) => {
      await fetchWithLoading(async () => {
         try {
            ...
            const result = await fetchPost("SP0108_S03", { data });
      ...
      <div className="bg-gray-100 rounded-lg p-5 search text-sm search flex justify-between items-center">
         <div className="grid grid-cols-4 gap-y-3 justify-start w-[70%]">
            <SelectSearch
                       ...
                       stringify={true}
                       param={{ coCd: "200",bpType : "ZZ0002", bpNm : '999', bpDiv: '999', billBpCd: userInfo.bpCd || "999" }}
                       procedure="ZZ_B_PO_BP_BILL"
            ...
            <SelectSearch
               ...
               addData={"999"}
               stringify={true}
               param={{ coCd: "200", billBpCd: userInfo.bpCd || "999" }}
